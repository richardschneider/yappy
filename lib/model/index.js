...
let model =
    ...
    .map(filename => {
        ...
        meta.collection = () => meta.db.collection(meta.collectionName, (err, collection) => {
            if (err) {
                log.error(`Failed to get collection '${meta.collectionName}'.`, meta, err);
                throw err;
            }
            log.debug(`Got collection '${meta.collectionName}'.`);
            return collection;
        });
        return meta;
    })
    .reduce((model, meta) => {
        ...
        log.debug(`Defined '${meta.collectionName}'.`);
        return model;
    }, {});

/*
 * Validate a resource, or list of resources, against its schema.
 */
model.validate = (resource, type, contentType, opts) => {
    // Check for JSON APIish search result.
    if (resource.data)
    ...
    if (contentType == 'application/schema+json') {
        // https://github.com/richardschneider/yappy/issues/99
        ...
        if (!schema) {
            ...
            catch (e) { log.error(`failed to find ${type} schema`) } // eat it
        }
    }
