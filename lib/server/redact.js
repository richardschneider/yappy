...
function redact (req, res, next) {
    ...
    if (req.body && Object.keys(req.body).length > 0 && (req.method == 'POST' || req.method == 'PUT' || req.method == 'PATCH')) {
        if (req.contains(redact.mask[0]))
            return next(new HttpError(422, "Cannnot create/update a redacted resource"));

        if (req.method == 'PATCH')
            return security.encryptJSONPatchAsync(req.body).then(() => next());
        else
            return security.encryptAsync(req.body).then(() => next());
    }
