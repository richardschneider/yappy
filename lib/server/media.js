...
router.get('/media/:id', function(req, res, next) {
    model
    .then(model => model.media.collection())
    .then(collection => collection.findByIdAsync(req.params.id))
    .then(media => {
        if (!media || media.metadata.tenantId != req.tenantId)
            return res.sendError(404, 'Not found');

        res
            .set('Last-Modified',
                new Date(media.uploadDate).toUTCString())
            .send(media)
            .end();
    });
});
