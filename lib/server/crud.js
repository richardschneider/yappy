...
app.use(require('./authorisation').allowsResourceAccess());
...
app.get('/:collectionName', function(req, res, next) {
    ...
    let options = {
        limit: Math.min(parseInt(req.query.n, 10) || max, max),
        skip: parseInt(req.query.o, 10) || 0, // TODO: doesn't scale well use range query
    };
    ...
    req.collection
        ...
        .then(function (docs) {
            ...
            if (true)
            {
                ...
                q.n = options.limit;
                q.o = 0;
                result.links.first = `${req.baseUrl}${req.path}?${querystring.stringify(q)}`;
            }
            if (options.limit <= docs.length)
            {
                ...
                result.links.next = `${req.baseUrl}${req.path}?${querystring.stringify(q)}`;
            }
            if (options.skip !=  0) {
                let q = extend({}, req.query);
                q.n = options.limit;
                q.o = Math.max(0, options.skip - options.limit);
                result.links.prev = `${req.baseUrl}${req.path}?${querystring.stringify(q)}`;
            }
