...
function isPermittedToView (user, resourceLink, jsonPointer) {
    ...
    let permission = `${scope}:${r.type}:view:${r.id}${xtras}`;
    return isPermitted(user, permission);
}
...
function authorisation (req, res, next) {
    ...
    req.user.demandViewPermission = (resourceLink, jsonPointer) => {
        let r = link.tryParse(resourceLink);
        if (!r)
            return;
