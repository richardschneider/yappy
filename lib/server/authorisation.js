...
/*
 * Determine if the user is permitted to perform the activity (permission).  First the user's
 * permissions are checked, then each of the user's roles are checked.
 */
function isPermitted (req, permission)
{
    let user = req.user;
    ...
    if (authz.considerSubject(user).isPermitted(permission))
        return true;
    if (user.roles) {
        for (let roleUrl of user.roles) {
            //console.log('checking that', user.email, 'as', roleUrl, 'is permitted', permission);
        }
    }
    return false;
}

function isPermittedToView (req, resourceLink) {
    let r = link.tryParse(resourceLink);
    if (!r)
        return true;

    let permission = `api:${r.type}:view:${r.id}`;
    return isPermitted(req, permission);
}
...
function authorisation (req, res, next) {
    req.user.isPermitted = permission => isPermitted(req, permission);
    req.user.isPermittedToView = resourceLink => isPermittedToView(req, resourceLink);
    req.user.demand = (activity, resourceLink, jsonPointer) => {
        let permission;
