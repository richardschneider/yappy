...
function successfulResponse(ctx)
{
    ...
    if (isJson(ctx)) {
        ...
        for (let body of content) {
            should.exist(body, 'null content is evil');
            should.exist(body._metadata, 'missing _metadata');
            should.exist(body._metadata.self, 'missing _metadata.self');
        }
        if (ctx.req.tenant.self_test.validateResource)
        {
...
function selftest(req, res, next) {
    ...
    res.send = function (body) {
        if (!req.tenant.self_test.validateResponse)
